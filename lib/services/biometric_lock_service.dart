...
class BiometricLockService {
    ...
    try {
      ...
      if (!supported || !canCheck || biometrics.isEmpty) {
        return const BiometricSupportState(
          isAvailable: false,
          label: 'Biometrics are not configured on this device.',
        );
      }
      ...
      return const BiometricSupportState(
        isAvailable: false,
        label: 'Biometrics cannot be verified at this time.',
      );
    }
  }
  ...
  Future<bool> authenticate() async {
    try {
      ...
      return await _auth.authenticate(
        localizedReason: 'Unlock VaultChat to access encrypted messages.',
  ...
  String _labelFor(List<BiometricType> biometrics) {
    if (biometrics.contains(BiometricType.face)) {
      return 'Unlock with Face Unlock';
    }
